...
const WithdrawRequestOtherRoute = express.Router();
...
WithdrawRequestOtherRoute.post("/:id",async(req,res)=>{
    try {
...
WithdrawRequestOtherRoute.get("/:id",async(req,res)=>{
    try {
        try {
            if (req.params.id === "admin") {
                const withdraw = await WithdrawModel1.find({  });
                return res.status(200).send(withdraw);
            }
            AccountNo = req.params.id;
            const withdraw = await WithdrawModel1.find({ IDNumber: AccountNo });
            return res.status(200).send(withdraw);
        } catch (error) {
...
WithdrawRequestOtherRoute.put('/:id/:withdrawid', async (req, res) => {
    try {
        const { status } = req.body;
        const accountId = req.params.id;
        const withdrawId = req.params.withdrawid;
        const withdraw = await WithdrawModel1.findOne({ IDNumber: accountId, withdraw_id: withdrawId });
...
module.exports=WithdrawRequestOtherRoute